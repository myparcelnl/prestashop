 ...
 */
namespace MyParcelModule\cweagans\Composer;
...
use MyParcelModule\Symfony\Component\Process\Process;
class Patches implements \MyParcelModule\Composer\Plugin\PluginInterface, \MyParcelModule\Composer\EventDispatcher\EventSubscriberInterface
{
    /**
     ...
     */
    public function activate(\MyParcelModule\Composer\Composer $composer, \MyParcelModule\Composer\IO\IOInterface $io)
    {
        $this->composer = $composer;
        $this->io = $io;
        $this->eventDispatcher = $composer->getEventDispatcher();
        $this->executor = new \MyParcelModule\Composer\Util\ProcessExecutor($this->io);
        $this->patches = array();
    ...
    {
        return array(
            \MyParcelModule\Composer\Script\ScriptEvents::PRE_INSTALL_CMD => array('checkPatches'),
            \MyParcelModule\Composer\Script\ScriptEvents::PRE_UPDATE_CMD => array('checkPatches'),
            \MyParcelModule\Composer\Installer\PackageEvents::PRE_PACKAGE_INSTALL => array('gatherPatches'),
            \MyParcelModule\Composer\Installer\PackageEvents::PRE_PACKAGE_UPDATE => array('gatherPatches'),
            ...
            \MyParcelModule\Composer\Installer\PackageEvents::POST_PACKAGE_INSTALL => array('postInstall', 10),
            \MyParcelModule\Composer\Installer\PackageEvents::POST_PACKAGE_UPDATE => array('postInstall', 10),
        );
    }
    /**
     ...
     * @param Event $event
     */
    public function checkPatches(\MyParcelModule\Composer\Script\Event $event)
    {
            ...
            foreach ($packages as $package) {
                if (!$package instanceof AliasPackage) {
                    ...
                    if ($has_patches && !$has_applied_patches || !$has_patches && $has_applied_patches || $has_patches && $has_applied_patches && $tmp_patches[$package_name] !== $extra['patches_applied']) {
                        $uninstallOperation = new \MyParcelModule\Composer\DependencyResolver\Operation\UninstallOperation($package, 'Removing package so it can be re-installed and re-patched.');
     ...
     */
    public function gatherPatches(\MyParcelModule\Composer\Installer\PackageEvent $event)
    {
        // If we've already done this, then don't do it again.
        if (isset($this->patches['_patchesGathered'])) {
            $this->io->write('<info>Patches already gathered. Skipping</info>', TRUE, \MyParcelModule\Composer\IO\IOInterface::VERBOSE);
            return;
        } elseif (!$this->isPatchingEnabled()) {
            $this->io->write('<info>Patching is disabled. Skipping.</info>', TRUE, \MyParcelModule\Composer\IO\IOInterface::VERBOSE);
            return;
        }
     ...
     * @throws \Exception
     */
    public function postInstall(\MyParcelModule\Composer\Installer\PackageEvent $event)
    {
        ...
        // Set up a downloader.
        $downloader = new \MyParcelModule\Composer\Util\RemoteFilesystem($this->io, $this->composer->getConfig());
        ...
        foreach ($this->patches[$package_name] as $description => $url) {
            ...
            try {
                $this->eventDispatcher->dispatch(NULL, new \MyParcelModule\cweagans\Composer\PatchEvent(\MyParcelModule\cweagans\Composer\PatchEvents::PRE_PATCH_APPLY, $package, $url, $description));
                $this->getAndApplyPatch($downloader, $install_path, $url, $package);
                $this->eventDispatcher->dispatch(NULL, new \MyParcelModule\cweagans\Composer\PatchEvent(\MyParcelModule\cweagans\Composer\PatchEvents::POST_PATCH_APPLY, $package, $url, $description));
     ...
     * @throws \Exception
     */
    protected function getPackageFromOperation(\MyParcelModule\Composer\DependencyResolver\Operation\OperationInterface $operation)
    {
     ...
     * @throws \Exception
     */
    protected function getAndApplyPatch(\MyParcelModule\Composer\Util\RemoteFilesystem $downloader, $install_path, $patch_url, \MyParcelModule\Composer\Package\PackageInterface $package)
    {
        // Local patch file.
    ...
    protected function executeCommand($cmd)
    {
        ...
        if ($this->io->isVerbose()) {
            ...
            $output = function ($type, $data) use($io) {
                if ($type == \MyParcelModule\Symfony\Component\Process\Process::ERR) {

...
class MyParcelCustomsItem
{
    ...
    public function isFullyFilledItem()
    {
        if ($this->getDescription() === null) {
            throw new \InvalidArgumentException('setDescription() must be set');
        }
        if ($this->getAmount() === null) {
            throw new \InvalidArgumentException('setAmount() must be set');
        }
        if ($this->getWeight() === null) {
            throw new \InvalidArgumentException('setWeight() must be set');
        }
        if ($this->getItemValue() === null) {
            throw new \InvalidArgumentException('setItemValue() must be set');
        }
        if ($this->getClassification() === null) {
            throw new \InvalidArgumentException('setClassification() must be set');
        }
        if ($this->getCountry() === null) {
            throw new \InvalidArgumentException('setCountry() must be set');
        }
        return true;
    }
    /**
    ...
    {
        if ($weight == 0) {
            throw new \InvalidArgumentException('Weight must be set for a MyParcel product');
        }

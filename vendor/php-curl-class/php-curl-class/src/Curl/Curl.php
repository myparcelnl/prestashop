...
class Curl
{
    ...
    public function buildPostData($data)
    {
        ...
        if (isset($this->headers['Content-Type']) && preg_match($this->jsonPattern, $this->headers['Content-Type']) && (is_array($data) || is_object($data) && interface_exists('JsonSerializable', false) && $data instanceof \JsonSerializable)) {
            $data = \MyParcelModule\Curl\Encoder::encodeJson($data);
    ...
    {
        $this->jsonDecoder = '\\MyParcelModule\\Curl\\Decoder::decodeJson';
    ...
    {
        $this->xmlDecoder = '\\MyParcelModule\\Curl\\Decoder::decodeXml';
    ...
    public function setDefaultDecoder($mixed = 'json')
    {
        if ($mixed === false) {
            ...
            if ($mixed === 'json') {
                $this->defaultDecoder = '\\MyParcelModule\\Curl\\Decoder::decodeJson';
            } elseif ($mixed === 'xml') {
                $this->defaultDecoder = '\\MyParcelModule\\Curl\\Decoder::decodeXml';
            }
        }
    }
    /**
    ...
    public function setUrl($url, $mixed_data = '')
    {
        ...
        if ($this->url === null) {
            $this->url = (string) new \MyParcelModule\Curl\Url($built_url);
        } else {
            $this->url = (string) new \MyParcelModule\Curl\Url($this->url, $built_url);
        }
    ...
    private function parseHeaders($raw_headers)
    {
        ...
        $http_headers = new \MyParcelModule\Curl\CaseInsensitiveArray();
    ...
    {
        $request_headers = new \MyParcelModule\Curl\CaseInsensitiveArray();
    ...
    private function parseResponseHeaders($raw_response_headers)
    {
        ...
        $response_headers = new \MyParcelModule\Curl\CaseInsensitiveArray();
    ...
    private function initialize($base_url = null)
    {
        ...
        $this->setOpt(CURLOPT_RETURNTRANSFER, true);
        $this->headers = new \MyParcelModule\Curl\CaseInsensitiveArray();

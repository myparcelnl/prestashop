...
use MyParcelModule\Curl\StrUtil;
class Url
{
    ...
    public static function removeDotSegments($input)
    {
        ...
        while (!empty($input)) {
            ...
            if (\MyParcelModule\Curl\StrUtil::startsWith($input, '../')) {
                $input = substr($input, 3);
            } elseif (\MyParcelModule\Curl\StrUtil::startsWith($input, './')) {
    ...
    private function absolutizeUrl()
    {
        ...
        if (!($this->relativeUrl === null)) {
            ...
            if (!\MyParcelModule\Curl\StrUtil::startsWith($r['path'], '/')) {

...
use MyParcelModule\Curl\StringUtil;
class Url
{
    ...
    public static function removeDotSegments($input)
    {
        ...
        while (!empty($input)) {
            ...
            if (\MyParcelModule\Curl\StringUtil::startsWith($input, '../')) {
                $input = substr($input, 3);
            } elseif (\MyParcelModule\Curl\StringUtil::startsWith($input, './')) {
                ...
                $input = substr($input, 3);
                $output = substr_replace($output, '', \MyParcelModule\Curl\StringUtil::reversePosition($output, '/'));
            } elseif ($input === '/..') {
                $input = '/';
                $output = substr_replace($output, '', \MyParcelModule\Curl\StringUtil::reversePosition($output, '/'));
            ...
            } elseif (!(($pos = \MyParcelModule\Curl\StringUtil::position($input, '/', 1)) === false)) {
        ...
        if (isset($r['scheme'])) {
            ...
            if ($r['authorized']) {
                ...
                if (!isset($r['path']) || $r['path'] === '') {
                    ...
                    if (\MyParcelModule\Curl\StringUtil::startsWith($r['path'], '/')) {
                        $target['path'] = self::removeDotSegments($r['path']);
                    } else {
                        $base = \MyParcelModule\Curl\StringUtil::characterReversePosition($b['path'], '/', true);
                        if ($base === false) {
                            $base = '';
                        }
